...
We will see a quick setup to configure the ``drone`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``drone`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``drone`` formulas under ``/srv/salt/drone/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/drone/parameters/dns:domain/
    ln -s dns:domain /srv/salt/drone/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/drone/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/drone/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``drone`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``drone`` formulas under ``/srv/salt/drone/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/drone/parameters/roles
...
We create a configuration for the role ``drone/server`` in ``/srv/salt/drone/parameters/roles/drone/server.yaml``:
...
We create another configuration for the role ``drone/client`` in ``/srv/salt/drone/parameters/roles/drone/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles drone/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles drone/client
...
.. code-block:: console
    ...
    mv /srv/salt/drone/parameters/dns:domain/  '/srv/salt/drone/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-drone-as-json:
      test.nop:
        - name: {{ drone | json }}
...
Here is an example based on `template-formula/drone/config/file.sls`_:
    ...
    drone-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='drone-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ drone.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``drone`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ drone['winner'] }}
...
.. _template-formula/drone/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/drone/config/file.sls
